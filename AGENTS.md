...
```bash
python -m py_compile jupyter_ui_tweak/__init__.py  # Check single file for syntax errors
```
...
```typescript
const PLUGIN_ID = 'jupyter_ui_tweak:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'jupyter_ui_tweak:open-panel';
  export const refreshData = 'jupyter_ui_tweak:refresh-data';
}
...
- **Output**: Creates files in `lib/` and `jupyter_ui_tweak/labextension/`
...
- Ask user to search for the extension ID (`jupyter_ui_tweak`) to see if it loaded
...
ls -la jupyter_ui_tweak/labextension/  # Should contain bundled extension
```
...
- **Plugin ID** (in `src/index.ts`): `'jupyter_ui_tweak:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'jupyter_ui_tweak:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-jupyter-ui-tweak-ClassName`
